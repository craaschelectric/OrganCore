...
`ORGAN_COMBINATION_MEDIA != COMBINATION_MEDIA_SPIFLASH`, together the guard
`ORGANCORE_HAS_REMAP_STORE`). In HW mode, SPI-flash media, the whole feature compiles out and
`applyRemaps()` reads the const `remapFrom[]`/`remapTo[]` exactly as before.
...
- **1.4.0** — **Builder piston assignment** (see the section above): a touchscreen
  ...
  SD-card media (guard `ORGANCORE_HAS_REMAP_STORE`). Enabling constants changed, all
  ...
  const source at compile time; HW and SPI-flash builds are unaffected.
